...
public class OrderUi_controller implements Initializable {
    ...
    @FXML private TableColumn<Order, Integer> ConfirmationColumn;
    @FXML private TableColumn<Order, Integer> SubscriberColumn;
    @FXML private TableColumn<Order, Date> PlacingDateColumn;
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        // ---- חיבור העמודות לשדות המחלקה Order ----
        ...
        ConfirmationColumn.setCellValueFactory(new PropertyValueFactory<>("confirmation_code"));
        SubscriberColumn.setCellValueFactory(new PropertyValueFactory<>("subscriber_id"));
        PlacingDateColumn.setCellValueFactory(new PropertyValueFactory<>("date_of_placing_order"));
    ...
    private void setupEditableColumns() {
        ...
        // עריכת קוד אישור
        ConfirmationColumn.setCellFactory(TextFieldTableCell.forTableColumn(new IntegerStringConverter()));
        ConfirmationColumn.setOnEditCommit(e -> {
            if (e.getNewValue() != null)
                e.getRowValue().setConfirmation_code(e.getNewValue());
        });

        // עריכת Subscriber ID
        SubscriberColumn.setCellFactory(TextFieldTableCell.forTableColumn(new IntegerStringConverter()));
        SubscriberColumn.setOnEditCommit(e -> {
            if (e.getNewValue() != null)
                e.getRowValue().setSubscriber_id(e.getNewValue());
        });

        // עריכת תאריך (במידה ותרצה — כרגע לא חובה)
        // PlacingDateColumn.setCellFactory(TextFieldTableCell.forTableColumn(new DateStringConverter()));
        // PlacingDateColumn.setOnEditCommit(e -> e.getRowValue().setDate_of_placing_order(e.getNewValue()));
    }
    ...
    private void handleUpdateOrder() {
        ...
        // מתחיל עריכה על עמודת Guests (או שתחליט על עמודה אחרת)
