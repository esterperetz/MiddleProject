...
import server.controller.OrderScheduler; // Added import
...
public class ServerLoginController {
    ...
    @FXML
    private TextField Scheme; // DB Schema (Database Name)
    @FXML
    ...
    void onConnect(ActionEvent event) {
        ...
        try {
            ...
            OrderScheduler.startOrderExpirationTask(); // Start the order expiration scheduler

            // 5. Transition to the main server screen: replace the Scene in the current
            // window
            ...
            // alert.setContentText("Could not connect to the database. Check credentials
            // and schema: \n" + e.getMessage());
            // alert.showAndWait();
        } catch (IOException e) {
            e.printStackTrace();
            ...
            // alert.setContentText("Could not load 'connections_to_server.fxml'. Check file
            // path:\n" + e.getMessage());
            // alert.showAndWait();
        } catch (Exception e) {
            e.printStackTrace();
