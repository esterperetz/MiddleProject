 ...
 * <p>
 * This class handles:
 * 1. Navigation to various manager sub-screens (Waiting List, Orders, Registration).
 * 2. Configuration of Standard Opening Hours.
...
public class ManagerOptionsController implements Initializable {
     ...
     * Sets up permissions, loads initial data, and initializes the lists.
     *
    ...
    public void initialize(URL location, ResourceBundle resources) {
        // --- 1. Permission Check ---
        // TODO: Replace with real logic (e.g., ClientUI.currentUser.getRole().equals("MANAGER"))
        isManager = true;

        if (isManager) {
            // Reveal the Reports button if the user has the correct permissions
            btnViewReports.setVisible(true);
            btnViewReports.setManaged(true);
        }

        // --- 2. Load Standard Hours ---
        // Pre-fill the text fields with the current hours from the server
        loadStandardHours();

        // --- 3. Initialize Special Dates List ---
        // Create the observable list and bind it to the ListView
        specialDatesModel = FXCollections.observableArrayList();
        listSpecialDates.setItems(specialDatesModel);

        // Load Mock Data (Simulating existing data from server)
    ...
    /**
     * Action handler to save the standard weekly opening hours.
     * Triggered by the "Save" button in the standard hours section.
     *
    ...
    /**
     * Adds a new special date/holiday to the list.
    ...
    /**
     * Removes the selected special date from the list.
     ...
     * Changes the text color based on whether it is an error or success.
     *
     ...
     * Only accessible if the user has Manager permissions.
     */
    @FXML
